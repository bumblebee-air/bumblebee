...
@section('page-content')
			...
			<div class="container-fluid">
				<div class="row">
					<div class="col-md-12">
								...
								<div class="col-12 col-xl-7 col-lg-8 col-md-9 col-sm-12">
									<div class="row justify-content-end mt-2 mt-xs-0 filterContrainerDiv mb-2 mt-1">
										...
										<div class="col-lg-3 col-md-3  col-sm-3 px-md-1">
											<a href="{{ url()->current() . '?export_type=exel' }}" class="btn-doorder-filter w-100"> Export list </a>
										</div>
									</div>
								</div>
							</div>
						</div>
						<form method="POST" id="assignOrdersDrivers" action="{{ url('doorder/assign_orders_drivers') }}"
							style="margin-bottom: 0 !important;">
							{{ csrf_field() }} <input type="hidden" name="selectedOrders" id="selectedOrders" :value="selectedOrders">
							<div class="card">
								...
								<div class="card-body">
									...
									<div class="table-responsive">
										<table id="driversTable" class="table table-no-bordered table-hover doorderTable " cellspacing="0"
											...
											<tbody>
												<tr v-for="driver in drivers" v-if="drivers.length > 0" class="order-row"
													...
													<td class="text-left"><span v-if="driver.in_duty" class="inDutyDriverSpan inDutyTrue"> <i
																class="fas fa-circle"></i>
														</span> <span v-else class="inDutyDriverSpan inDuty0"> <i class="fas fa-circle"></i>
													...
													<td class="actionsTd"><button type="button" class="edit" @click="openDriver(driver.id)">
															<img src="{{ asset('images/doorder-new-layout/edit-icon.png') }}">
														</button>
														<button type="button" class="remove" @click="clickDeleteDriver(driver.id)">
										...
										<nav aria-label="pagination" class="float-right">{{-- {{$clients->links('vendor.pagination.bootstrap-4')}} --}}</nav>
									</div>

								</div>
							</div>
							<div class="card" v-if="selectedOrders.length > 0" style="background-color: transparent; box-shadow: none;">
								<div class="card-body p-0">
									<div class="container w-100" style="max-width: 100%">

										<div class="row justify-content-center ">
											<div class="col-xl-3 col-lg-4  col-md-4 col-sm-5 px-md-1 text-center w-100">
						...
						<form method="GET" id="goToMapViewForm" action="{{ url('doorder/view_route_optimization_map') }}"></form>
					</div>
				</div>
			</div>
	...
	<!-- delete driver modal -->
	<div class="modal fade" id="delete-driver-modal" tabindex="-1" role="dialog" aria-labelledby="delete-deliverer-label"
		aria-hidden="true">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div>
						...
						<form method="POST" id="delete-driver" action="{{ url('doorder/driver/delete') }}"
							style="margin-bottom: 0 !important;">
