      ...
      <div class="order-details-cart" id="expanded-card" :style="'height: '+minHeight+'em'" v-touch:start="touchStartHandler" v-touch:moving="movingHandler" v-touch:end="movedHandler">
        ...
        <div class="order-details-cart-actions">
          <div class="row accept-reject-container" v-if="job_data.status == 'ready'">
            <div class="col-md-12">
              <div class="row">
                <button :class="job_data.is_contacted == 1 ? 'col-10 btn btn-lg doorder-btn block ' : 'btn btn-lg doorder-btn block '" @click="applyForJob" v-if="!apply_job">
                  Apply for this job
                </button>
                <button :class="job_data.is_contacted == 1 ? 'col-10 btn btn-lg doorder-btn block ' : 'btn btn-lg doorder-btn block '" @click="submitApplication" v-else>
                  Send application
                </button>
