...
@section('content')
    <div class="container" id="app">
        @if(!$job->confirmation_status)
            <div class="row">
                <div class="col-md-12 delivery-confirmation-logo">
                    <img src="{{asset('images/unified/Logo.png')}}" alt="DoOrder">
                </div>
                ...
                <div class="col-md-12">
                    ...
                    <form id="confirmation_form" action="{{url("customer/job/$job->customer_confirmation_code")}}" method="post">
                        @csrf
                        <input type="hidden" name="customer_confirmation_code" value="{{ $job->customer_confirmation_code }}">
                        <input type="hidden" name="engineer_confirmation_code" v-model="engineer_confirmation_code">
                    </form>
                </div>
            </div>
        @else
        ...
        var app = new Vue({
            ...
            methods: {
                onDecode(decodedString) {
                    if (decodedString) {
                        this.engineer_confirmation_code = decodedString;
