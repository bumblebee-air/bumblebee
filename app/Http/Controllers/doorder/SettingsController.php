...
class SettingsController extends Controller
{
    ...
    public function getSettings()
    {
        ...
        $admins = User::where('user_role', 'client')->whereHas('client', function ($q) use ($the_client) {
            ...
            ->get();
        $retailers = User::where('user_role', 'retailer')->get();
        ...
        $retailers = $retailers->map(function ($item) {
            return $retailer = [
                'id' => $item->id,
                'label' => $item->name,
                'customLabel' => "Retailer, $item->name"
            ];
        });
        ...
        $view = strpos(strtolower($the_client->name), 'doorder') ? 'admin.doorder.settings.index' : 'admin.garden_help.setting.index' ;
        return view($view, [
